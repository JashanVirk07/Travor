...
const AdminDashboardPage = () => {
              ...
              <td style={styles.td}>
                <span style={{...styles.status, 
                  background: booking.status === 'confirmed' ? '#d1fae5' : 
                              ...
                              booking.status === 'refund_pending' ? '#fee2e2' : '#fef3c7',
                  color: booking.status === 'confirmed' ? '#065f46' : 
                         booking.status === 'completed' ? '#1e40af' : 
                         booking.status === 'refund_pending' ? '#991b1b' : '#92400e'
                }}>
                  {booking.status === 'refund_pending' ? 'Refund Requested' : booking.status}
                </span>
              </td>
            </tr>
          ))}
  ...
  const RefundsTab = () => (
    <div style={styles.tableContainer}>
      {refunds.length === 0 ? (
          <div style={{padding: '40px', textAlign: 'center', color: '#666'}}>No pending refund requests</div>
      ) : (
        <table style={styles.table}>
            <thead>
            <tr>
            ...
            {refunds.map(r => (
                ...
                <td style={styles.td}>{r.id.slice(0,8)}...</td>
                <td style={styles.td}>{r.travelerName}</td>
                <td style={styles.td}>{r.refundReason || 'No reason'}</td>
                <td style={styles.td}>${r.totalPrice}</td>
                <td style={styles.td}>
                    <button onClick={() => handleApproveRefund(r)} style={styles.approveBtn}>
                    💰 Approve Refund
                    </button>
                </td>
                </tr>
    ...
    <div style={styles.container}>
      <div style={styles.sidebar}>
        ...
        <nav style={styles.nav}>
          ...
          <button style={activeTab === 'users' ? styles.navItemActive : styles.navItem} onClick={() => setActiveTab('users')}>
            👥 Users & Guides
          </button>
          <button style={activeTab === 'bookings' ? styles.navItemActive : styles.navItem} onClick={() => setActiveTab('bookings')}>
            📅 Bookings
          </button>
...
const styles = {
  ...
  sidebar: { width: '250px', background: '#1f2937', color: 'white', padding: '20px', display: 'flex', flexDirection: 'column' },
  sidebarHeader: { marginBottom: '40px', paddingBottom: '20px', borderBottom: '1px solid #374151' },
  nav: { display: 'flex', flexDirection: 'column', gap: '10px' },
  navItem: { padding: '12px', background: 'transparent', border: 'none', color: '#9ca3af', textAlign: 'left', cursor: 'pointer', fontSize: '16px', borderRadius: '8px', transition: '0.2s' },
  navItemActive: { padding: '12px', background: COLORS.primary, border: 'none', color: 'white', textAlign: 'left', cursor: 'pointer', fontSize: '16px', borderRadius: '8px', fontWeight: 'bold' },
  content: { flex: 1, padding: '40px', overflowY: 'auto' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' },
  adminInfo: { color: '#666' },
  statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '24px' },
  statCard: { background: 'white', padding: '24px', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' },
  statLabel: { color: '#666', fontSize: '14px', marginBottom: '8px' },
  statValue: { fontSize: '32px', fontWeight: 'bold', color: '#111' },
  tableContainer: { background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' },
  table: { width: '100%', borderCollapse: 'collapse' },
  th: { textAlign: 'left', padding: '16px', background: '#f9fafb', borderBottom: '1px solid #e5e7eb', fontSize: '14px', fontWeight: '600', color: '#374151' },
  tr: { borderBottom: '1px solid #e5e7eb' },
  td: { padding: '16px', fontSize: '14px', color: '#4b5563' },
  roleGuide: { background: '#e0e7ff', color: '#4338ca', padding: '4px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: '600' },
  roleUser: { background: '#f3f4f6', color: '#374151', padding: '4px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: '600' },
